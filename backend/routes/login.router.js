...
router.route('/checkIfAliveSession').post((req,res) => {
    ...
    UserSession.find({token: token})
        .then((response) =>{
            if(response[0] && response[0].validUntil > moment()){
...
router.route('/verifyCredentials').post((req,res) => {
    ...
    User.find({userName: userName})
        .then((userDataFromDb) => {
            ...
            let newUserSession = new UserSession({userName: userName, validUntil: moment().add(sessionTime,'hours'), token: sessionToken })
